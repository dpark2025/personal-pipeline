...
export class PPMCPTools {
    ...
    async searchRunbooks(args) {
        ...
        const cacheKey = createCacheKey('runbooks', JSON.stringify({ alert_type, severity, affected_systems, context }));
        if (this.cacheService) {
    ...
    async getDecisionTree(args) {
        ...
        const cacheKey = createCacheKey('decision_trees', JSON.stringify({ alert_context, has_agent_state: !!current_agent_state }));
        if (this.cacheService) {
            const cachedResult = await this.cacheService.get(cacheKey);
            if (cachedResult) {
    ...
    async searchKnowledgeBase(args) {
        ...
        const cacheKey = createCacheKey('knowledge_base', JSON.stringify({ query, categories, max_age_days, max_results }));
        if (this.cacheService) {
            const cachedResult = await this.cacheService.get(cacheKey);
            if (cachedResult) {
