...
import { validateRequest, handleAsyncRoute, createSuccessResponse, createErrorResponse, } from './middleware.js';
import { createSuccessResponseWithCorrelation, createErrorResponseWithCorrelation, } from './correlation.js';
...
export function createAPIRoutes(options) {
    ...
    router.post('/search', validateRequest({
        type: 'object',
        properties: {
        ...
        try {
            ...
            const cacheKey = cacheService
                ? createCacheKey(cacheKeyPrefix, JSON.stringify(mcpRequest))
                : null;
            ...
            res.status(errorDetails.httpStatus).json(createErrorResponseWithCorrelation(errorDetails.code, errorDetails.message, req, {
                execution_time_ms: Math.round(executionTime),
    ...
    router.post('/runbooks/search', validateRequest({
        type: 'object',
        properties: {
        ...
        try {
            ...
            const cacheKey = cacheService
                ? createCacheKey('runbooks', JSON.stringify({
    ...
    router.get('/runbooks/:id', handleAsyncRoute(async (req, res) => {
        const startTime = performance.now();
        ...
        try {
            ...
            if (!specificRunbook) {
                res
                    .status(404)
                    .json(createErrorResponseWithCorrelation('RUNBOOK_NOT_FOUND', `Runbook with ID '${runbookId}' not found`, req, { execution_time_ms: Math.round(executionTime) }));
                return;
            }
            ...
            res
                .status(statusCode)
                .json(createErrorResponseWithCorrelation('RUNBOOK_RETRIEVAL_FAILED', error instanceof Error ? error.message : 'Runbook retrieval operation failed', req, { execution_time_ms: Math.round(executionTime) }));
        }
    }));
    router.get('/runbooks', handleAsyncRoute(async (req, res) => {
        const startTime = performance.now();
        try {
            ...
            res
                .status(statusCode)
                .json(createErrorResponseWithCorrelation('RUNBOOK_LISTING_FAILED', error instanceof Error ? error.message : 'Runbook listing operation failed', req, { execution_time_ms: Math.round(executionTime) }));
        }
    }));
    router.post('/decision-tree', validateRequest({
        type: 'object',
        properties: {
            alert_context: {
                type: 'object',
        ...
        try {
            ...
            const executionTime = performance.now() - startTime;
            const statusCode = getErrorStatusCode(error);
            res.status(statusCode).json(createErrorResponse('DECISION_TREE_FAILED', error instanceof Error ? error.message : 'Decision tree retrieval failed', {
                execution_time_ms: Math.round(executionTime),
            }));
        }
    }));
    router.get('/procedures/:id', handleAsyncRoute(async (req, res) => {
        const startTime = performance.now();
        ...
        try {
            ...
            if (!runbookId || !stepName) {
                res
                    .status(400)
                    .json(createErrorResponse('INVALID_PROCEDURE_ID', 'Procedure ID must be in format: runbook_id_step_name'));
                return;
            }
            ...
            const executionTime = performance.now() - startTime;
            const statusCode = getErrorStatusCode(error);
            res.status(statusCode).json(createErrorResponse('PROCEDURE_RETRIEVAL_FAILED', error instanceof Error ? error.message : 'Procedure retrieval failed', {
                execution_time_ms: Math.round(executionTime),
            }));
        }
    }));
    router.post('/escalation', validateRequest({
        type: 'object',
        properties: {
            severity: {
                type: 'string',
        ...
        try {
            ...
            const executionTime = performance.now() - startTime;
            res.status(getErrorStatusCode(error)).json(createErrorResponse('ESCALATION_FAILED', 'Escalation path retrieval failed', {
                execution_time_ms: Math.round(executionTime),
            }));
        }
    }));
    router.get('/sources', handleAsyncRoute(async (req, res) => {
        const startTime = performance.now();
        try {
            ...
            const executionTime = performance.now() - startTime;
            res.status(getErrorStatusCode(error)).json(createErrorResponse('SOURCES_LISTING_FAILED', 'Sources listing failed', {
                execution_time_ms: Math.round(executionTime),
            }));
        }
    }));
    router.post('/feedback', validateRequest({
        type: 'object',
        properties: {
        ...
        try {
            ...
            res
                ...
                .json(createErrorResponseWithCorrelation('FEEDBACK_RECORDING_FAILED', 'Feedback recording failed', req, { execution_time_ms: Math.round(executionTime) }));
        }
    }));
    router.get('/health', handleAsyncRoute(async (req, res) => {
        const startTime = performance.now();
        try {
            ...
            res
                .status(getErrorStatusCode(error))
                .json(createErrorResponseWithCorrelation('HEALTH_CHECK_FAILED', 'API health check failed', req, { execution_time_ms: Math.round(executionTime) }));
        }
    }));
    router.get('/performance', handleAsyncRoute(async (req, res) => {
        const startTime = performance.now();
        try {
            ...
            res
                .status(getErrorStatusCode(error))
                .json(createErrorResponseWithCorrelation('PERFORMANCE_METRICS_FAILED', 'Performance metrics retrieval failed', req, { execution_time_ms: Math.round(executionTime) }));
        }
    }));
