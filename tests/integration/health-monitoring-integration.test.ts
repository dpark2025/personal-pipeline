...
describe('Health Monitoring Integration Tests', () => {
      ...
      try {
        ...
        const detailedHealth = {
          ...
          performance: {
            ...
            tool_performance: Array.from(performanceMonitor.getToolMetrics().entries()).map(
              ([name, metrics]) => ({
                tool_name: name,
                ...metrics,
              })
  ...
  describe('Basic Health Endpoints', () => {
    ...
    it('should provide performance health information', async () => {
      ...
      const response = await request(app).get('/health/performance').expect(200);
  ...
  describe('Monitoring Status and Alerts', () => {
    ...
    it('should list only active alerts when requested', async () => {
      const response = await request(app).get('/monitoring/alerts?active=true').expect(200);
      ...
      const allAlertsResponse = await request(app).get('/monitoring/alerts').expect(200);
  ...
  describe('System Degradation Scenarios', () => {
    ...
    it('should handle monitoring service alerts during degradation', async () => {
      ...
      const errorAlert = response.body.alerts.find(
        (alert: any) => alert.title === 'High Error Rate'
      );
    ...
    it('should report performance degradation in health checks', async () => {
      ...
      const slowToolAlert = response.body.report.alerts.find(
        (alert: string) =>
  ...
  describe('Health Check Error Handling', () => {
    it('should handle health check errors gracefully', async () => {
      ...
      const response = await request(app).get('/health/error-test').expect(500);
  ...
  describe('Metrics Collection and Reporting', () => {
    it('should collect comprehensive system metrics', async () => {
      ...
      const operations = [
        {
          name: 'search_runbooks',
          args: { alert_type: 'test1', severity: 'high', systems: ['db'] },
        },
      ...
      expect(response.body.performance.tool_performance.length).toBeGreaterThanOrEqual(
        operations.length
      );
    });
