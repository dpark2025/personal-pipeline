...
const mockDocuments: SearchResult[] = [
  {
    ...
    content:
      'This document covers how to manage disk space issues, including monitoring disk usage, clearing temporary files, and expanding storage.',
    source: 'test',
    source_type: 'file',
  ...
  },
  {
    ...
    content:
      'Guide for diagnosing and resolving memory pressure issues in production systems. Covers memory monitoring, process analysis, and optimization techniques.',
    source: 'test',
    source_type: 'file',
  ...
  },
  {
    ...
    content:
      'Troubleshooting database connectivity problems including timeout issues, connection pool management, and network configuration.',
    source: 'test',
    source_type: 'file',
  ...
  },
  {
    ...
    content:
      'Comprehensive guide for optimizing CPU performance, including profiling tools, bottleneck identification, and optimization strategies.',
    source: 'test',
    source_type: 'file',
  ...
  },
  {
    ...
    content:
      'Network issue diagnosis and resolution procedures, covering latency problems, packet loss, and connectivity failures.',
    source: 'test',
    source_type: 'file',
...
(skipTests ? describe.skip : describe)('SemanticSearchEngine', () => {
  ...
  test('should index documents successfully', async () => {
    ...
    assert(
      status.embeddingCacheSize >= mockDocuments.length,
      `Expected cache size >= ${mockDocuments.length}, got ${status.embeddingCacheSize}`
    );
  });
  ...
  test('should perform semantic search with relevant results', async () => {
    ...
    for (let i = 1; i < results.length; i++) {
      assert(
        results[i - 1].confidence_score >= results[i].confidence_score,
  ...
  test('should search runbooks with context', async () => {
    ...
    const runbooks = await engine.searchRunbooks('disk_full', 'critical', ['web-server'], {
  ...
  test('should handle concurrent searches efficiently', async () => {
    ...
    assert(
      avgTimePerSearch < 200,
      `Average search time ${avgTimePerSearch.toFixed(2)}ms should be <200ms`
    );
  });
  ...
  test('should maintain memory efficiency', async () => {
    ...
    assert(
      memoryUsage < 50,
      `Memory usage ${memoryUsage.toFixed(2)}MB should be reasonable for test data`
    );
...
(skipTests ? describe.skip : describe)('SemanticSearchEngine Performance Benchmarks', () => {
  ...
  test('should meet embedding generation performance target', async () => {
    ...
    assert(
      embeddingTime < 100,
      `Embedding generation time ${embeddingTime.toFixed(2)}ms should be <100ms (target: <50ms)`
    );
  });
  ...
  test('should scale efficiently with document count', async () => {
    ...
    assert(
      searchTime < 300,
      `Search time ${searchTime.toFixed(2)}ms should be <300ms with 50 documents`
    );
    assert(results.length > 0, 'Should return relevant results');

    console.log(
      `Performance with 50 documents: Index=${indexTime.toFixed(2)}ms, Search=${searchTime.toFixed(2)}ms`
    );
  });
});
