...
- **Caching**: Memory-only by default, Redis optional via `REDIS_URL` environment variable
...
- `REDIS_URL` - Redis connection string (optional - enables Redis caching when set)
- Other source-specific credentials

### Redis Configuration
**Redis is DISABLED by default** to eliminate unwanted connections and dependencies:

**Enable Redis via Environment Variable:**
```bash
export REDIS_URL="redis://localhost:6379"
npm start  # Now uses Redis + memory hybrid caching
```

**Enable Redis via Configuration File:**
```yaml
cache:
  strategy: hybrid  # or redis_only
  redis:
    enabled: true
...
**Redis Testing:**
```bash
# Regular tests (Redis integration tests skipped)
npm test

# Include Redis integration tests (requires Redis server)
npm run test:redis

# Redis tests in watch mode
...
- Redis integration tests (opt-in with `TEST_REDIS=true` or `REDIS_URL` env var)
- Memory-only caching tests run by default (no Redis required)
