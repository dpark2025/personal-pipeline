...
class SessionManager {
    constructor() {
        ...
        this.sessionFile = path_1.default.join(__dirname, '../../.session', 'explorer-session.json');
        this.sessionData = this.getDefaultSessionData();
    }
    async loadSession() {
        try {
            const sessionDir = path_1.default.dirname(this.sessionFile);
            await promises_1.default.mkdir(sessionDir, { recursive: true });
            const data = await promises_1.default.readFile(this.sessionFile, 'utf-8');
            ...
            console.log(chalk_1.default.gray(`📂 Session loaded: ${this.sessionData.history.length} commands, ${this.sessionData.favorites.length} favorites`));
        }
        catch (error) {
            this.sessionData = this.getDefaultSessionData();
            console.log(chalk_1.default.gray('📂 Starting new session'));
        }
    }
    async saveSession() {
        try {
            ...
            const sessionDir = path_1.default.dirname(this.sessionFile);
            await promises_1.default.mkdir(sessionDir, { recursive: true });
            await promises_1.default.writeFile(this.sessionFile, JSON.stringify(this.sessionData, null, 2));
    ...
    async clearSession() {
        this.sessionData = this.getDefaultSessionData();
        try {
            await promises_1.default.unlink(this.sessionFile);
        }
        catch (error) {
        }
    }
    async exportSession(filePath) {
        const exportPath = filePath || path_1.default.join(__dirname, '../../exports', `session-export-${Date.now()}.json`);
        const exportData = {
            ...this.sessionData,
            exportedAt: new Date(),
