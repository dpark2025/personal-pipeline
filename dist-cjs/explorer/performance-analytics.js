...
class PerformanceAnalytics {
    ...
    formatResponseTime(responseTime) {
        ...
        const chalkFunction = chalk_1.default[rating.color];
        return chalkFunction(`${responseTime}ms ${rating.emoji}`);
    }
    formatSuccessRate(successRate) {
        if (successRate >= 0.95) {
            return chalk_1.default.green(`${(successRate * 100).toFixed(1)}% 🌟`);
        }
        else if (successRate >= 0.9) {
            return chalk_1.default.cyan(`${(successRate * 100).toFixed(1)}% ✅`);
        }
        else if (successRate >= 0.8) {
            return chalk_1.default.yellow(`${(successRate * 100).toFixed(1)}% ⚠️`);
        }
        else {
            return chalk_1.default.red(`${(successRate * 100).toFixed(1)}% ❌`);
        }
    }
    formatRequestCount(count) {
        if (count > 1000) {
            return chalk_1.default.cyan(`${(count / 1000).toFixed(1)}K requests`);
        }
        else if (count > 100) {
            return chalk_1.default.blue(`${count} requests`);
        }
        else if (count > 10) {
            return chalk_1.default.gray(`${count} requests`);
        }
        else {
            return chalk_1.default.gray(`${count} request${count === 1 ? '' : 's'}`);
        }
    }
    generateId() {
