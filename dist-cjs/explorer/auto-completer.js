...
class AutoCompleter {
    ...
    formatSuggestions(suggestions, _context = '') {
        if (suggestions.length === 0) {
            return chalk_1.default.gray('No suggestions available');
        }
        const formatted = suggestions.map((suggestion, _index) => {
            const tool = this.tools[suggestion];
            if (tool) {
                return `${chalk_1.default.cyan(suggestion)} ${chalk_1.default.gray('- ' + tool.description)}`;
            }
            return chalk_1.default.cyan(suggestion);
        });
