...
class ParameterHandler {
    ...
    async collectParameters(schema, askQuestion) {
        ...
        if (optionalParams.length > 0) {
            ...
            console.log(chalk_1.default.gray('Press Enter to skip any optional parameter'));
        ...
        while (attempts < maxAttempts) {
            ...
            try {
                ...
                if (!input.trim()) {
                    if (isRequired) {
                        if (attempts < maxAttempts) {
                            console.log(chalk_1.default.red('❌ This parameter is required. Please provide a value.'));
                            continue;
                        }
                        else {
                            const defaultValue = this.getDefaultValue(toolName, paramName, paramDef);
                            console.log(chalk_1.default.yellow(`⚠️  Using default value: ${JSON.stringify(defaultValue)}`));
                            return defaultValue;
                        }
                    }
                    else {
                        console.log(chalk_1.default.gray('⏭️  Skipped (optional)'));
                        return undefined;
                    }
                }
                ...
                if (validation.isValid) {
                    ...
                    console.log(chalk_1.default.green(`✅ ${paramName}: ${JSON.stringify(validation.transformedValue)}`));
                    return validation.transformedValue;
                }
                else {
                    console.log(chalk_1.default.red(`❌ ${validation.error}`));
                    if (validation.suggestion) {
                        console.log(chalk_1.default.yellow(`💡 Suggestion: ${validation.suggestion}`));
                    }
                    if (attempts >= maxAttempts) {
                        console.log(chalk_1.default.red('❌ Max attempts reached. Using default value.'));
                        return this.getDefaultValue(toolName, paramName, paramDef);
                    }
                }
            }
            catch (error) {
                console.log(chalk_1.default.red('❌ Input cancelled. Using default value.'));
                return this.getDefaultValue(toolName, paramName, paramDef);
            }
        }
        return undefined;
    ...
    displayParameterInfo(paramName, paramDef, isRequired) {
        const requiredTag = isRequired ? chalk_1.default.red('[REQUIRED]') : chalk_1.default.gray('[OPTIONAL]');
        const typeTag = chalk_1.default.cyan(`[${paramDef.type.toUpperCase()}]`);
        console.log(`\n${chalk_1.default.bold(paramName)} ${requiredTag} ${typeTag}`);
        console.log(chalk_1.default.gray(`  ${paramDef.description}`));
    }
    showSuggestions(toolName, paramName, paramDef) {
        ...
        if (suggestions.length > 0) {
            ...
            suggestions.slice(0, 5).forEach((suggestion, index) => {
                console.log(chalk_1.default.gray(`    ${index + 1}. ${suggestion}`));
            });
        }
        if (paramDef.examples && paramDef.examples.length > 0) {
            ...
            paramDef.examples.slice(0, 3).forEach((example, index) => {
                ...
                console.log(chalk_1.default.gray(`    ${index + 1}. ${formatted}`));
            });
        }
    }
    buildPrompt(paramName, paramDef, isRequired, attempt) {
        let prompt = chalk_1.default.cyan(`Enter ${paramName}`);
        if (paramDef.type === 'array') {
            prompt += chalk_1.default.gray(' (comma-separated)');
        }
        else if (paramDef.type === 'object') {
            prompt += chalk_1.default.gray(' (JSON format)');
        }
        else if (paramDef.type === 'boolean') {
            prompt += chalk_1.default.gray(' (true/false, yes/no)');
        }
        if (!isRequired) {
            prompt += chalk_1.default.gray(' (or press Enter to skip)');
        }
        if (attempt > 1) {
            prompt += chalk_1.default.yellow(` [Attempt ${attempt}/3]`);
        }
        prompt += ': ';
        return prompt;
    }
