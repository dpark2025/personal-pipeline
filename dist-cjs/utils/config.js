...
class ConfigManager {
    ...
    async loadConfig() {
        try {
            ...
            const validatedConfig = index_js_1.AppConfig.parse(config);
            ...
            if (error instanceof Error && 'code' in error && error.code === 'ENOENT') {
                logger_js_1.logger.warn('Configuration file not found, creating default configuration');
    ...
    addSource(sourceConfig) {
        ...
        if (existingIndex >= 0) {
            ...
            logger_js_1.logger.info('Updated existing source configuration', {
                sourceName: sourceConfig.name,
            });
        }
        else {
            this.config.sources.push(sourceConfig);
            logger_js_1.logger.info('Added new source configuration', {
                sourceName: sourceConfig.name,
            });
        }
    }
    removeSource(sourceName) {
        ...
        if (removed) {
            logger_js_1.logger.info('Removed source configuration', { sourceName });
        }
        return removed;
    }
    updateServerConfig(serverConfig) {
        ...
        logger_js_1.logger.info('Updated server configuration', { changes: serverConfig });
    }
    async createDefaultConfig() {
        const defaultConfig = {
            ...
            cache: {
                ...
                strategy: process.env.CACHE_STRATEGY || 'hybrid',
                memory: {
    ...
    async watchConfig(callback) {
        logger_js_1.logger.info('Configuration file watching is disabled in this implementation', {
            configPath: this.configPath,
        });
        void callback;
    }
...
async function createSampleConfig(outputPath = './config/config.sample.yaml') {
    const sampleConfig = {
        server: {
            port: 3000,
            host: 'localhost',
    ...
    logger_js_1.logger.info('Sample configuration created', { outputPath });
}
