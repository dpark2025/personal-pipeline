...
exports.intelligentCaching = intelligentCaching;
exports.warmCriticalCache = warmCriticalCache;
exports.analyzeCachePerformance = analyzeCachePerformance;
const logger_js_1 = require("../utils/logger.js");
function intelligentCaching(cacheService) {
    return (req, res, next) => {
        if (!cacheService) {
            next();
            return;
        ...
        void (async () => {
            try {
                ...
                if (cachedResponse) {
                    logger_js_1.logger.info('Cache hit for API request', {
                        path: req.path,
                        method: req.method,
                ...
                res.json = function (body) {
                    if (body && body.success !== false && res.statusCode < 400) {
                        ...
                        void cacheService
                            ...
                            .then(() => {
                            logger_js_1.logger.debug('Cached API response', {
                                path: req.path,
                                cacheKey: cacheKey.substring(0, 50),
                                ttl,
                                strategy: cacheStrategy,
                            ...
                            .catch((cacheError) => {
                            logger_js_1.logger.warn('Failed to cache API response', {
...
async function warmCriticalCache(cacheService, mcpTools) {
    if (!cacheService || !mcpTools) {
        return;
    }
    logger_js_1.logger.info('Starting cache warming for critical endpoints');
    try {
        ...
        for (const scenario of criticalScenarios) {
            try {
                ...
                if (mcpResult && !mcpResult.isError) {
                    ...
                    logger_js_1.logger.debug('Warmed runbook cache', {
                ...
                logger_js_1.logger.warn('Failed to warm runbook cache', {
                    scenario,
        ...
        for (const query of commonQueries) {
            try {
                ...
                if (mcpResult && !mcpResult.isError) {
                    ...
                    await cacheService.set(cacheKey, mcpResult, ttl);
                    logger_js_1.logger.debug('Warmed knowledge base cache', {
                ...
                logger_js_1.logger.warn('Failed to warm knowledge base cache', {
                    query,
                    error: error instanceof Error ? error.message : String(error),
                });
            }
        }
        logger_js_1.logger.info('Cache warming completed');
    }
    catch (error) {
        logger_js_1.logger.error('Cache warming failed', {
            error: error instanceof Error ? error.message : String(error),
        });
    }
}
function analyzeCachePerformance(cacheService) {
