...
exports.transformRestRequest = transformRestRequest;
exports.transformMCPResponse = transformMCPResponse;
exports.transformRunbookSearchResponse = transformRunbookSearchResponse;
exports.transformSearchResultsResponse = transformSearchResultsResponse;
exports.transformProcedureResponse = transformProcedureResponse;
exports.transformEscalationResponse = transformEscalationResponse;
exports.validateMCPResponse = validateMCPResponse;
exports.sanitizeResponseData = sanitizeResponseData;
const logger_js_1 = require("../utils/logger.js");
const perf_hooks_1 = require("perf_hooks");
function transformRestRequest(toolName, requestBody, context) {
    ...
    try {
        ...
        switch (toolName) {
            ...
            case 'record_resolution_feedback':
                ...
                break;
            default:
                logger_js_1.logger.warn('Unknown tool name for request transformation', { toolName });
                transformedRequest = baseRequest;
        }
        const transformTime = perf_hooks_1.performance.now() - startTime;
        if (transformedRequest._metadata) {
            transformedRequest._metadata.transform_time_ms = Math.round(transformTime);
        }
        logger_js_1.logger.debug('Request transformation completed', {
            toolName,
        ...
        const transformTime = perf_hooks_1.performance.now() - startTime;
        logger_js_1.logger.error('Request transformation failed', {
            toolName,
...
function transformSearchKnowledgeBaseRequest(body) {
    ...
    if (queryMetrics.suspiciousPatterns.length > 0) {
        logger_js_1.logger.warn('Suspicious query patterns detected', {
...
function transformMCPResponse(mcpResult, context) {
    const transformStartTime = perf_hooks_1.performance.now();
    try {
        ...
        if (typeof content === 'string') {
            try {
                ...
                logger_js_1.logger.warn('MCP response JSON parsing failed, treating as plain text', {
        ...
        const totalTime = context?.startTime ? perf_hooks_1.performance.now() - context.startTime : undefined;
        const transformTime = perf_hooks_1.performance.now() - transformStartTime;
        ...
        const transformTime = perf_hooks_1.performance.now() - transformStartTime;
        logger_js_1.logger.error('MCP response transformation failed', {
            toolName: context?.toolName,
...
function transformRunbookSearchResponse(content) {
...
function transformSearchResultsResponse(content) {
...
function transformProcedureResponse(content) {
...
function transformEscalationResponse(content) {
    const response = createGenericMCPResponse(content);
...
function sanitizeRequestBody(body, toolName) {
    ...
    dangerousFields.forEach(field => {
        if (field in sanitized) {
            ...
            logger_js_1.logger.warn('Removed dangerous field from request', { field, toolName });
        }
    });
...
function validateMaxAge(maxAge) {
    ...
    logger_js_1.logger.warn('Invalid max_age_days value, ignoring', { maxAge });
    return undefined;
}
...
function validateAlertType(alertType) {
    ...
    if (!knownPatterns.some(pattern => normalized.includes(pattern))) {
        logger_js_1.logger.info('Unknown alert type pattern detected', { alertType: normalized });
    }
    return normalized;
}
...
function validateMCPResponse(mcpResult) {
    try {
        ...
        logger_js_1.logger.error('MCP response validation failed', { error });
        return false;
    }
}
