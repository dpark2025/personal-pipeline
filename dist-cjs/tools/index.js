...
class PPMCPTools {
    ...
    async clearCacheByType(contentType) {
        if (this.cacheService) {
            await this.cacheService.clearByType(contentType);
        }
    }
    async handleToolCall(request) {
        const timer = new performance_js_1.PerformanceTimer();
        const performanceMonitor = (0, performance_js_1.getPerformanceMonitor)();
        let isError = false;
        try {
            logger_js_1.logger.info(`Handling tool call: ${request.params.name}`, {
    ...
    getRecordResolutionFeedbackTool() {
        return {
    ...
    async searchRunbooks(args) {
        ...
        const cacheKey = (0, cache_js_1.createCacheKey)('runbooks', JSON.stringify({ alert_type, severity, affected_systems, context }));
        if (this.cacheService) {
            const cachedResult = await this.cacheService.get(cacheKey);
            if (cachedResult) {
                logger_js_1.logger.debug('Cache hit for runbook search', {
                    alert_type,
                    severity,
        ...
        for (const adapter of adapters) {
            try {
                const runbooks = await adapter.searchRunbooks(alert_type, severity, affected_systems, context);
                allRunbooks.push(...runbooks);
            }
            catch (error) {
                logger_js_1.logger.error(`Error searching runbooks in adapter ${adapter.getConfig().name}:`, error);
            }
        }
        allRunbooks.sort((a, b) => (b.metadata?.confidence_score || 0) - (a.metadata?.confidence_score || 0));
        const result = {
            success: true,
        ...
        if (this.cacheService && allRunbooks.length > 0) {
            ...
            logger_js_1.logger.debug('Cached runbook search result', {
                alert_type,
                severity,
    ...
    async getDecisionTree(args) {
        const { alert_context, current_agent_state } = args;
        const cacheKey = (0, cache_js_1.createCacheKey)('decision_trees', JSON.stringify({ alert_context, has_agent_state: !!current_agent_state }));
        if (this.cacheService) {
            const cachedResult = await this.cacheService.get(cacheKey);
            if (cachedResult) {
                logger_js_1.logger.debug('Cache hit for decision tree', {
        ...
        if (this.cacheService) {
            await this.cacheService.set(cacheKey, result);
            logger_js_1.logger.debug('Cached decision tree result', {
                alert_context: alert_context?.alert_type || 'unknown',
                cache_key: `${cacheKey.identifier.substring(0, 50)}...`,
            });
        }
    ...
    async getProcedure(args) {
        ...
        const cacheKey = (0, cache_js_1.createCacheKey)('procedures', JSON.stringify({ runbook_id, step_name }));
        if (this.cacheService) {
            const cachedResult = await this.cacheService.get(cacheKey);
            if (cachedResult) {
                logger_js_1.logger.debug('Cache hit for procedure', {
                    runbook_id,
                    step_name,
        ...
        if (this.cacheService) {
            await this.cacheService.set(cacheKey, result);
            logger_js_1.logger.debug('Cached procedure result', {
                runbook_id,
                step_name,
                cache_key: cacheKey.identifier,
            });
    ...
    async searchKnowledgeBase(args) {
        ...
        const cacheKey = (0, cache_js_1.createCacheKey)('knowledge_base', JSON.stringify({ query, categories, max_age_days, max_results }));
        if (this.cacheService) {
            const cachedResult = await this.cacheService.get(cacheKey);
            if (cachedResult) {
                logger_js_1.logger.debug('Cache hit for knowledge base search', {
        ...
        for (const adapter of adapters) {
            try {
                ...
                logger_js_1.logger.error(`Error searching in adapter ${adapter.getConfig().name}:`, error);
            }
        }
    ...
    async recordResolutionFeedback(args) {
        const { runbook_id, procedure_id, outcome, resolution_time_minutes, notes } = args;
        logger_js_1.logger.info('Recording resolution feedback', {
            runbook_id,
            procedure_id,
            outcome,
            resolution_time_minutes,
