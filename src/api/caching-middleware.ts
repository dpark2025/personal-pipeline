...
export function intelligentCaching(cacheService?: any) {
  return (req: Request, res: Response, next: NextFunction): void => {
    ...
    const cacheable = req.method === 'GET' || 
      (req.method === 'POST' && isCacheableEndpoint(req.path));
...
function generateCacheKey(req: Request): string {
  ...
  const paramsKey = req.method === 'GET' 
...
export async function warmCriticalCache(cacheService: any, mcpTools: any): Promise<void> {
  ...
  try {
    ...
    const criticalScenarios = [
      ...
      { alert_type: 'service_down', severity: 'critical', affected_systems: ['production', 'web', 'api'] },
      { alert_type: 'memory_pressure', severity: 'critical', affected_systems: ['production', 'database'] },
      ...
      { alert_type: 'network_error', severity: 'critical', affected_systems: ['network', 'connectivity'] },
      { alert_type: 'load_balancer_failure', severity: 'critical', affected_systems: ['infrastructure', 'web'] },
      
      // Security incidents
      { alert_type: 'security_breach', severity: 'critical', affected_systems: ['security', 'auth'] },
      { alert_type: 'authentication_failure', severity: 'high', affected_systems: ['auth', 'security'] },
...
function calculateAverageResponseTimeSaved(stats: any): number {
  ...
  if (stats.strategy_stats) {
    Object.entries(stats.strategy_stats).forEach(([strategy, stratStats]: [string, any]) => {
      const estimatedTime = typicalResponseTimes[strategy as keyof typeof typicalResponseTimes] || 300;
 ...
 */
function analyzeStrategyPerformance(stats: any): Record<string, { hit_rate: number; requests: number }> {
...
function identifyPeakUsageHours(stats: any): number[] {
  ...
  const avgUsage = Object.values(hourlyUsage).reduce((sum: number, usage: any) => sum + usage, 0) / 24;
