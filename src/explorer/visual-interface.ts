...
╚══════════════════════════════════════════════════════════════════════════════╝
  ...
  showToolsMenu(
    ...
    Object.entries(categories).forEach(([category, categoryTools]) => {
      ...
      categoryTools.forEach(tool => {
        ...
        console.log(
          `${favoriteIcon} ${chalk.cyan(index.toString().padStart(2))}. ${chalk.bold(tool.name)}`
        );
  ...
  showFavorites(favorites: Favorite[]): void {
    ...
    if (favorites.length === 0) {
      console.log(
        chalk.gray(
          'No favorites saved yet. Add some by using tools and selecting "y" when prompted.\n'
        )
      );
      return;
    }
    ...
    favorites.forEach((favorite, index) => {
      ...
      console.log(
        `${chalk.cyan((index + 1).toString().padStart(2))}. ${chalk.bold(favorite.toolName)}`
      );
  ...
  showAnalytics(analytics: PerformanceAnalytics): void {
    ...
    if (Object.keys(toolStats).length > 0) {
      ...
      const statsTable = Object.entries(toolStats).map(([tool, stats]: [string, any]) => [
        ...
        stats.successRate >= 0.9
          ? chalk.green(`${(stats.successRate * 100).toFixed(1)}%`)
          : stats.successRate >= 0.7
            ? chalk.yellow(`${(stats.successRate * 100).toFixed(1)}%`)
    ...
    if (recentActivity.length > 0) {
      ...
      recentActivity.slice(-5).forEach(activity => {
        ...
        console.log(
          `${statusIcon} ${chalk.bold(activity.tool)} - ${activity.responseTime}ms (${timeAgo})`
        );
      });
    }
  ...
  private showQuickStats(analytics: PerformanceAnalytics, favoritesCount: number): void {
    ...
    console.log(
      `${chalk.gray('Quick Stats: ')}${totalRequests} requests • ` +
