...
FORCE_CONFIG_OVERWRITE=${FORCE_CONFIG_OVERWRITE:-false}
...
EOF
    ...
    # Check if existing config has custom configurations before overwriting
    local main_config="${DEMO_DIR}/config/config.yaml"
    local should_overwrite=true
    
    # Force overwrite if explicitly requested
    if [ "$FORCE_CONFIG_OVERWRITE" = true ]; then
        log_info "Force overwrite enabled, replacing existing config"
        should_overwrite=true
    elif [ -f "$main_config" ]; then
        # Check for signs of custom configuration (non-demo sources, custom categories, etc.)
        if grep -q "categories:" "$main_config" 2>/dev/null; then
            local existing_sources=$(grep -c "name:" "$main_config" 2>/dev/null || echo "0")
            local demo_sources=3  # We know demo config has 3 sources
            
            # If existing config has categories and different number of sources, it's likely customized
            if [ "$existing_sources" -ne "$demo_sources" ] || ! grep -q "demo-runbooks" "$main_config" 2>/dev/null; then
                log_warning "Existing config appears to have custom configurations"
                log_info "Preserving existing config.yaml (demo config available as config/demo-config.yaml)"
                should_overwrite=false
            elif grep -q "Generated by setup-demo.sh" "$main_config" 2>/dev/null; then
                # Config was generated by demo script, safe to overwrite
                log_info "Existing config was generated by demo script, updating..."
                should_overwrite=true
            else
                # Has categories but might be manually configured
                log_warning "Existing config has categories but may be manually configured"
                log_info "Backing up existing config to config.yaml.backup before overwrite"
                cp "$main_config" "${main_config}.backup"
                log_success "Backup created: ${main_config}.backup"
                should_overwrite=true
            fi
        fi
    fi
    
    # Copy demo config to main config file if safe to do so
    if [ "$should_overwrite" = true ]; then
        cp "$config_file" "$main_config"
        log_info "Demo configuration activated as main config"
    else
        log_info "Using existing config.yaml, demo config available at: $config_file"
    fi
}
...
start_server() {
    ...
    # Stop any existing server to ensure config reload
    if curl -s "http://localhost:${SERVER_PORT}/health" &> /dev/null; then
        log_info "Stopping existing server to reload configuration..."
        pkill -f "node.*index.js" || pkill -f "tsx.*index.ts" || true
        sleep 2
    fi
...
run_benchmarks() {
    ...
    # Run minimal benchmark with server reuse to avoid port conflicts
    if node scripts/benchmark.js --concurrent 2 --duration 5 --target 200 --reuse-server --port "${SERVER_PORT}" 2>/dev/null; then
        log_success "Performance benchmarks completed"
    else
        # Fallback with even faster settings
        if node scripts/benchmark.js --concurrent 1 --duration 3 --target 300 --reuse-server --port "${SERVER_PORT}" --verbose; then
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo "  --force-config-overwrite  Force overwrite existing config.yaml"
            ...
            echo "  FORCE_CONFIG_OVERWRITE=true    Force config overwrite"
            exit 0
            ;;
        *)
